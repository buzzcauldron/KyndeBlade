...
|------------|---------------|---------------------------|
| Core game / save | `Code/Core/Game/`, `Code/Core/Save/` | `scripts/bootstrap/save_service.gd`, `scripts/bootstrap/game_state.gd` |
| Input | `Code/Core/Input/` | `scripts/bootstrap/input_map_setup.gd`, `project.godot` input map |
| Narrative | `Code/Core/Narrative/`, `StoryBeat` assets | `scripts/ui/story_arrival_screen.gd`, `exported_from_unity.json` `arrival_beat_*` per location |
| Map / locations | `Code/Core/Map/`, `Code/Combat/Map/`, `WorldMapManager`, `LocationNode` | `scripts/hub/hub_map.gd`, `scripts/world/unity_export_data.gd`, `data/slice_locations.json`, `data/exported_from_unity.json`, **`data/world/locations_registry.json`**, **`scripts/world/`**, **`WorldNav` autoload** — see [`KyndeBlade_Godot/docs/GAME_SKELETON.md`](../KyndeBlade_Godot/docs/GAME_SKELETON.md) |
| Combat loop / UI | `Code/Combat/*.cs`, `Code/Combat/UI/` | `scripts/combat/combat_manager.gd`, `scripts/combat/combat_root.gd`, `scenes/combat.tscn` |
...
| Audio buses (shell) | Unity mixers (future parity) | `scripts/bootstrap/audio_bus_setup.gd` autoload (`Music`, `SFX`) |
