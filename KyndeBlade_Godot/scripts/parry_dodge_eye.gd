...
func _process(_delta: float) -> void:
	if _combat == null:
		visible = false
		return
	if _combat.state == CombatManager.State.DEFENSE_WINDUP:
		visible = true
		var real := _combat.defensive_preview_is_real()
		react_label.text = "Gather… %s" % ("TRUE EDGE" if real else "FEINT")
		pupil_root.scale = Vector2(PUPIL_OPEN_SCALE, PUPIL_OPEN_SCALE)
		var lid := EYELID_CLOSE_AMOUNT * 0.25
